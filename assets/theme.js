...
function closeSidebar() {
  ...
  document.querySelectorAll(".submenu-drawer.active").forEach(sub => {
...
overlay.addEventListener("click", () => {
  // Find the last opened submenu (if multiple)
  const activeSubmenus = document.querySelectorAll(".submenu-drawer.active");
  ...
  if (lastSubmenu) {
    ...
    // If no submenu open, close the sidebar
    closeSidebar();
  }
});
...
// Back button (close submenu drawer)
document.querySelectorAll(".submenu-drawer .back-btn").forEach(btn => {
  btn.addEventListener("click", function () {
    this.closest(".submenu-drawer").classList.remove("active");
  });
});
